...
//导入试题
if($_GET[action]=="loadquestion"){
   $mark = $db->query_first("SELECT * FROM ".$db_prefix."setmark");

   $contstr = array();
   $contstr[] = array('选择试题文件:','left','10%');
   $contstr[] = array(makeinput('file','questionfile',80,""));
   $header = array('导入试题',2);
   $titles = array();
   $footer = array('<input type="submit" name="submit" value="执行导入">','center');
   echo "<form name='form' action='$selfurl' method='post'><input name='action' value='doloadquestion' type='hidden'>";
   maketablev($header,$titles,$contstr,$footer);
   echo "</form>";
   
}
if($_POST[action]=="doloadquestion"){
	//打开文件
	$filename = $_POST["questionfile"];
	...
	while(!feof($file)) {
		...
		$quesion = explode("#",$row);
		$sqltitle = "INSERT INTO `test_title`(`threadid`, `title`, `choicetype`, `answer`) VALUES ('$quesion[0]','$quesion[1]','$quesion[2]','$quesion[3]')";	
		$db->query($sqltitle);
		$sqltitle = "select id from `test_title` where `threadid`=$quesion[0] and `title`='$quesion[1]'";
		$result = $db->query($sqltitle);
		$quesionId = mysql_fetch_array($result)["id"];
		//写入到选项
		$sqlchoice = "INSERT INTO `test_choice`(`choice`, `extends`, `IsDefault`) VALUES ('$quesion[4]','$quesionId',NULL)";
		$db->query($sqlchoice);
		$sqlchoice = "INSERT INTO `test_choice`(`choice`, `extends`, `IsDefault`) VALUES ('$quesion[5]','$quesionId',NULL)";
		$db->query($sqlchoice);
		$sqlchoice = "INSERT INTO `test_choice`(`choice`, `extends`, `IsDefault`) VALUES ('$quesion[6]','$quesionId',NULL)";
		$db->query($sqlchoice);
		$sqlchoice = "INSERT INTO `test_choice`(`choice`, `extends`, `IsDefault`) VALUES ('$quesion[7]','$quesionId',NULL)";
		$db->query($sqlchoice);
	}
	fclose($file);	
	msg("导入试题成功。","./test_thread.php?action=loadquestion");
